...
async def rebuild_graph(tenant_id, kb_id, exclude_rebuild=None):
    ...
    def _merge_subgraph_doc(d: dict):
        if not d:
            return
    ...
    # Temporary fix for Elasticsearch deep pagination limits: use scan to iterate all docs.
    if getattr(settings.docStoreConn, "db_type", lambda: "")() == "elasticsearch":
        try:
            ...
            for hit in scan(
                ...
                d = hit.get("_source") or {}
                merged_graph = _merge_subgraph_doc(d)
                if merged_graph is not None:
                    graph = merged_graph
        except Exception as e:
            logging.warning(f"rebuild_graph scan failed, falling back to paging: {e}")
        else:
    ...
    # Fallback: paging (may hit max_result_window on large datasets)
